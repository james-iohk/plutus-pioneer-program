...
mkPolicy pkh deadline ctx = traceIfFalse "missing signature" isSigned  &&
                            traceIfFalse "deadline passed - too late" checkDeadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    isSigned :: Bool
    isSigned =  txSignedBy (scriptContextTxInfo ctx) pkh
